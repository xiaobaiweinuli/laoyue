...
def target_scan(url,target_id):
    try:
        ...
        if 'profile_id' in response.text and 'target_id' in response.text:
            print(
                target_id,
                '添加到AWVS扫描成功',
                url,
    ...
    try:
        ...
        for url in add_list1:
            if '_' not in url:
                ...
                add_log = requests.post(
                    f'{awvs_url}/api/v1/targets/add',
        ...
        while target_id_list:
            ...
            if int(vul_sum) > int(temp_sum) and len(result) != 0:
                ...
                if new_sum > 30:
                    message_push = (
                        (
                            (
                                f'{message_push}亲爱的主人,本次{tag}新增加漏洞数量为:{str(new_sum)}'
                                + '\n'
                                + '新增加高危数量:'
                                + str(new_high)
                                + '\n'
                    ...
                    print(message_push)
                    message_length = len(message_push)
                    ...
                    try:
                        ...
                        try:
                            ...
                            mid = len(message_push) // 2
                            push_wechat_group(message_push[:mid])
                            push_wechat_group(message_push[mid:])
                        except:
                            print('xxxxxxxxxxxxx12121')
                            mid = len(message_push) // 3
                            push_wechat_group(message_push[:mid])
            ...
            #状态检测看看有没有超出扫描限制
            stats_result = requests.get(
                url=f"{awvs_url}/api/v1/me/stats",
                headers=headers,
                verify=False,
            )
            ...
            if int(scan_num) < int(scan_count):
                ...
                try:
                    if '_' not in add_url:
                        ...
                        with open('./result/awvslist/cache.txt', 'a', encoding='utf-8') as caches_file:
...
def push_wechat_group(content):
    ...
    if resp.json()["errcode"] != 0:
        raise ValueError(f"push wechat group failed, {resp.text}")
    # except Exception as e:
    #     print('webhook',e)
...
def first_push():
    ...
    init_medium_count = 0
    init_low_count = 0
    get_target_url = f'{awvs_url}/api/v1/vulnerability_types?l=100&q=status:open;severity:1,2,3;'
    ...
    for r in result['vulnerability_types']:
        if int(r['severity']) > 0 and 'SSL' not in r['name'] and 'TLS' not in r['name'] and 'SPDY' not in r['name'] and 'CORS' not in r['name'] and 'Cookie' not in r['name']:
            if int(r['severity']) == 1:
                ...
                level_vul = '高危'
            message_push = (
                f'{message_push}漏洞等級: {level_vul}漏洞: '
                + r['name']
                + '数量: '
                + str(r['count'])
            ) + '\n'
...
def monitor_time_scans():
    resp = requests.get(
        f"{awvs_url}/api/v1/scans?l=100&q=status:processing;",
        headers=headers,
        verify=False,
    )
    ...
    for scan in scan_list:
        try:
            ...
            if status == 'processing':
                start_time = scan['current_session']['start_date'][:-6]
...
def stop_scan(scan_dict):
    for key in scan_dict:
        try:
            resp = requests.post(
                f"{awvs_url}/api/v1/scans/{key}/abort",
                headers=headers,
                timeout=30,
                verify=False,
            )
            ...
            if resp.status_code == 204:
                print("\n", scan_dict[key], f" 扫描任务超过{str(scan_time)}" + "，已中止扫描任务\n")
        except Exception as e:
            print("[*] 中止扫描任务时出现错误\n", e)
